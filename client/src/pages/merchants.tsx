  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <Header
          title="Merchants"
          description="Manage game merchants and view transaction statistics"
          action={{
            label: "Add Merchant",
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {topLoading ? (
              Array.from({ length: 3 }).map((_, i) => (
                <Card key={i} className="discord-darker border-gray-700">
              ...
              topMerchants.slice(0, 3).map((merchant: any, index) => {
                ...
                return (
                  <Card key={merchant.id} className="discord-darker border-gray-700">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <div className="text-lg font-bold text-white">${merchant.totalRevenue}</div>
                          <div className="text-xs discord-text">#{index + 1}</div>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {merchants.map((merchant: any) => {
                    ...
                    return (
                      <div key={merchant.id} className="p-4 bg-gray-800/50 rounded-lg hover:bg-gray-800 transition-colors">
                        ...
                        {merchant.description && (
                          <p className="text-sm discord-text mb-3">{merchant.description}</p>
                        )}
                        <div className="text-xs discord-text">
                          Created {formatDistanceToNow(new Date(merchant.createdAt), { addSuffix: true })}
                        </div>
                      </div>
                    );
                  })}
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="discord-darker border-gray-700">
          <DialogHeader>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea placeholder="Enter merchant description" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
